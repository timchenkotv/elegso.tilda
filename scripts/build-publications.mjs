...
function absoluteFeedHtml(html) {
  return html.replace(/\b(href|src)=(["'])\/(?!\/)/g,`$1=$2${origin}/`)
    .replace(/\bsrcset=(["'])(.*?)\1/g,(_match,quote,value)=>`srcset=${quote}${value.replace(/(^\s*|,\s*)\/(?!\/)/g,`$1${origin}/`)}${quote}`);
}
...
const registeredArticles = config.publications.map(meta => {
  ...
  const article = {...meta,...item};
  const publishedAt = article.publishedAt ?? config.section.publishedAt;
  // Section updates must not silently re-date previously published articles.
  const modifiedAt = article.modifiedAt ?? publishedAt;
  if (!Number.isFinite(Date.parse(publishedAt)) || !Number.isFinite(Date.parse(modifiedAt)) || Date.parse(modifiedAt)<Date.parse(publishedAt)) throw new Error('Invalid article dates '+meta.slug);
  if (article.author && (!article.author.name?.trim() || !article.author.url)) throw new Error('Incomplete article author '+meta.slug);
...
function footerNav(html) {
  const card = footerCard('articles');
...
function shell(title,description,url,body,item=null) {
  ...
  for(const [attr,name,value] of [['name','description',description],['property','og:title',title],['property','og:description',description],['property','og:url',origin+url],['property','og:type',item?'article':'website'],['property','og:image',origin+(item?.image||catalogueCover.image)],['name','robots','index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1']]) head=meta(head,attr,name,value);
  if(item){head=meta(head,'property','article:published_time',item.publishedAt);head=meta(head,'property','article:modified_time',item.modifiedAt);}
  ...
  graph.push(item?{'@type':'Article','@id':origin+url+'#article',headline:item.title,description,mainEntityOfPage:origin+url,url:origin+url,inLanguage:'ru-RU',datePublished:item.publishedAt,dateModified:item.modifiedAt,image:[origin+item.image],author:authorSchema(item),publisher:{'@id':origin+'/#organization'},articleSection:item.category,wordCount:item.words,citation:item.sources.map(s=>s.url)}:{'@type':'CollectionPage',name:title,url:origin+url,description,inLanguage:'ru-RU',mainEntity:{'@type':'ItemList',itemListElement:articles.map((a,i)=>({'@type':'ListItem',position:i+1,url:origin+a.url,name:a.title}))}});
...
feed.posts=articles.map(a=>({uid:a.source.legacyUid,title:a.title,descr:a.description,text:'',url:a.url,directlink:a.url,directtarget:'',date:a.publishedAt,published:a.publishedAt,image:a.image,imagealt:a.imageAlt,thumb:a.image,postparts:[],stats:{views:0,likes:0}}));
...
for(const name of ['sitemap.xml','sitemap.base.xml']){const p=path.join(web,name);let sitemap;try{sitemap=await fs.readFile(p,'utf8')}catch{continue}const rows=[...sitemap.matchAll(/<url>[\s\S]*?<\/url>/g)].map(m=>m[0]).filter(r=>!r.includes('<loc>'+origin+'/articles/'));
  ...
  for(let i=0;i<rows.length;i++){if([...changed].some(url=>rows[i].includes('<loc>'+url+'</loc>')))rows[i]=rows[i].replace(/<lastmod>[^<]+<\/lastmod>/,`<lastmod>${collectionModifiedAt.slice(0,10)}</lastmod>`);}
  for(const entry of [{url:'/articles/',modifiedAt:collectionModifiedAt},...articles]) rows.push(`<url><loc>${origin+entry.url}</loc><lastmod>${entry.modifiedAt.slice(0,10)}</lastmod></url>`);
